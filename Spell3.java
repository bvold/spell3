...
final class Spell3 {
	...
	// Note '&' in the following declarlation
	// Note: " " needs to be handled separately as a special case.
	private final String ALPHABET = "abcdefghijklmnopqrstuvwxyz&";
	private static ArrayList<String> corrections = new ArrayList<String>(); // List of potential corrections
	...
	public boolean phonetic_check(StringBuffer oword) { // 'o' for original word
		if (Dictionary.contains(oword)) {
			...
			String[] diphthong_array = getSize(oword);
			// ---------------------------------------------------
			...
			for (int i = 0; i < d_count; i++) {
				System.out.println("diph_array[" + i + "] = " + diphthong_array[i]);
			}
			...
			for (int i = 0; i < d_count; i++) {
				...
				diph_names = Spellings.getProperty(diphthong_array[i]);
			...
			ArrayList<String> unused_prefix = new ArrayList<>();
			...
			for (int i = 0; i < unused_prefix.size(); i++) {
				diph_bucket[0].remove(unused_prefix.get(i)); // Remove it, it can't start any words
			}
			...
			while (!done && (curr_max <= d_count)) { // Could a sentinel be used?
				...
				if (poss4pos[curr_pos].hasMoreElements()) {
					...
					for (int i = 0; i < curr_max + 1; i++) {
						if (curr_word[i] == null) {
							System.out.println("Word contains NULL, stripping -->");
						}
						else {
						  word += curr_word[i];
						}
			...
			// Will be done when poss4pos[dcount-1] wraps around.
			// This may need to be broken up into smaller chunks / routines FIXME
			check_loop: while (!done) { // Could a sentinel be used?
				if (poss4pos[curr_pos].hasMoreElements()) {
					curr_word[curr_pos] = (String) poss4pos[curr_pos].nextElement();
					word = ""; // reset for this iter.
					...
					if (word.contains("NULL")) {
					    System.out.println("Word contained NULL, stripping");
						word = word.replaceAll("NULL", "");
						System.out.println("Word after replace = " + word);
					}
	...
	private String[] getSize(StringBuffer oword) {
		...
		String diphthong_array[] = new String[50]; // Shouldn't ever go this high.
